...
bool SystemService::verifyPassword(const std::string& password, const std::string& hash)
{
    ...
    SHA256(reinterpret_cast<const unsigned char*>(password.c_str()), password.size(), digest);
...
SystemService::SsoValue SystemService::decodeSsoValue(const std::string& raw)
{
    SsoValue result;
    // 找第一个冒号
    auto p1 = raw.find(':');
    if (p1 == std::string::npos) return result;
    // 找第二个冒号
    auto p2 = raw.find(':', p1 + 1);
    if (p2 == std::string::npos) return result;

    result.accountToken = raw.substr(0, p1);
    result.username     = raw.substr(p1 + 1, p2 - p1 - 1);
    result.domainId     = raw.substr(p2 + 1);
    result.valid =
        !result.accountToken.empty() && !result.username.empty() && !result.domainId.empty();
    return result;
}
 ...
 */
void SystemService::registerLicense(const std::string& consumerKey,
                                    const std::string& consumerSecret, TokenCallback onSuccess,
                                    ErrorCallback onError)
{
    ...
    redisClient_->set(
        redisKey,
        consumerKey,
        [token, onSuccess, onError](bool ok) {
            if (!ok) {
...
void SystemService::loginUser(const std::string& accountToken, const std::string& username,
                              const std::string& password, LoginCallback onSuccess,
                              ErrorCallback onError)
{
    // 1. 验证account_token 是否存在于Redis
    redisClient_->get(
        accountTokenKey(accountToken),
        [this, accountToken, username, password, onSuccess, onError](
            ...
            userRepo_->findUserById(
                username,
                [this, accountToken, username, password, onSuccess, onError](
                    std::optional<drogon_model::myapp::Users> userOpt) {
...
/** 保活: 刷新两个Token的TTL */
void SystemService::keepAlive(const std::string& accountToken, const std::string& ssoCookie,
...
void SystemService::validateSession(const std::string& accountToken, const std::string& ssoCookie,
                                    ValidateCallback onSuccess, ErrorCallback onError)
{
    redisClient_->get(ssoKey(ssoCookie),
                      [accountToken, onSuccess, onError](std::optional<std::string> val) {
                          ...
                          auto sso = SystemService::decodeSsoValue(*val);
                          if (!sso.valid) {
                              onError("Corrupted session data", 500);
                              return;
                          }
                          if (sso.accountToken != accountToken) {
                              onError("Token mismatch", 401);
                              return;
                          }

                          // ✅ 返回 domainId 和 username 给调用方
                          SessionInfo info;
                          info.domainId = sso.domainId;
                          info.username = sso.username;
                          onSuccess(info);
                      });
}
