...
public:
    ...
    SystemService(std::shared_ptr<interfaces::IUserRepository> userRepo,
                  std::shared_ptr<interfaces::IRedisClient> redisClient, LicenseMap licenses)
        : userRepo_(userRepo)
        , redisClient_(redisClient)
        , licenses_(std::move(licenses))
    {}

    void registerLicense(const std::string& consumerKey, const std::string& consumerSecret,
                         TokenCallback onSuccess, ErrorCallback onError) override;

    void loginUser(const std::string& accountToken, const std::string& username,
                   const std::string& password, LoginCallback onSuccess,
                   ErrorCallback onError) override;

    void keepAlive(const std::string& accountToken, const std::string& ssoCookie,
                   SuccessCallback onSuccess, ErrorCallback onError) override;

    void validateSession(const std::string& accountToken, const std::string& ssoCookie,
                         ValidateCallback onSuccess, ErrorCallback onError) override;
...
private:
    ...
    static std::string ssoKey(const std::string& cookie) { return "sso:" + cookie; }

    // sso 编解码: accountToken:username:domainId
    static std::string encodeSsoValue(const std::string& accountToken, const std::string& username,
                                      const std::string& domainId)
    {
        return accountToken + ":" + username + ":" + domainId;
    }
    ...
    static SsoValue decodeSsoValue(const std::string& raw);
